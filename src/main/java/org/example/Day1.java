...
public class Day1 {
    ...
    static long aoc1(String input) {
        long[] elfCalList = getElfCalList(input);
        int max = 0;
    ...
    static long aoc1a(String input) {
        ...
        for (int i = 0; i < getElfCalList(input).length; i++) {
            pq.add(new Elf(getElfCalList(input)[i], i));
        }
        long out = 0L;
        for (int i = 0; i < 3; i++) {
            Elf elf = pq.poll();
//            System.out.println(elf);
    ...
    private static long[] getElfCalList(String input) {
        String[] elfLoad = input.split("""


                """);
        return Arrays.stream(elfLoad)
                .map(elf -> {
