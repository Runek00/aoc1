...
public class Day13 {
    ...
    static int aoc13a(String input) {
        input += """
                                
                [[2]]
                [[6]]
                """;
        List<Packet> packets = Arrays.stream(input.split("\n"))
                .filter(s -> s.length() >= 2)
                .map(Packet::new)
                .sorted()
                .toList();
        ...
        for (int i = 0; i < packets.size(); i++) {
            if (packets.get(i).row().equals("[[2]]") || packets.get(i).row().equals("[[6]]")) {
                out *= (i + 1);
            }
        }
        return out;
    ...
    record Packet(String row) implements Comparable<Packet> {
        @Override
        public int compareTo(Packet o) {
            return correctOrder(this.row(), o.row());
        }
    }
